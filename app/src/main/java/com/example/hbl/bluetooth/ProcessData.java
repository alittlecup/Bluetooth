...
public class ProcessData {
    ...
    public static String toHexString(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            String s4;
            int chi = s.charAt(i) & MotionEventCompat.ACTION_MASK;
            if (chi < 16) {
    ...
    public static String toHexbyte(byte[] bytein, int count) {
        ...
        for (int i = 0; i < count; i++) {
            String s4;
            int chi = bytein[i] & MotionEventCompat.ACTION_MASK;
            if (chi < 16) {
                s4 = new StringBuilder(String.valueOf('0')).append(Integer.toHexString(chi)).toString();
            } else {
                s4 = Integer.toHexString(chi);
    ...
    public static byte[] StrToHexbyte(String stringin) {
        ...
        for (int i = 0; i < bArr.length / 2; i++) {
            byte ch = bArr[i * 2];
            byte cl = bArr[(i * 2) + 1];
            int chh = ch & MotionEventCompat.ACTION_MASK;
            if (chh >= 97) {
                chh -= 87;
            } else if (65 <= chh) {
                chh -= 55;
            ...
            int chl = cl & MotionEventCompat.ACTION_MASK;
            if (chl >= 97) {
                chl -= 87;
            } else if (65 <= chl) {
                chl -= 55;
    ...
    public static int toint(byte[] bytein) {
        return (bytein[0] * 256) + (bytein[1] & MotionEventCompat.ACTION_MASK);
    }
